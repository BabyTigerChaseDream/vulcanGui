...
def RunFindRevCmd(FindRevQueue,CLQueue,cmdline,cmdfinish_evt):
    ...
    for line in proc.stdout:
        ...
        if line.decode("utf-8").strip().isdigit():
            CLQueue.put(line.decode("utf-8"))
            print(' CL Num:',line.decode("utf-8"))
    proc.stdout.close()
...
def GenerateCmdList(CLQueue, cmdtemplate):
    ...
    while True:
        try:
            CLData = CLQueue.get(block=False)
            print('GenCmdList', CLData)
            cmdlist.append(CLData)
...
##### prameter check #####
def isRevRange(inputStr):
    return bool(re.search(r'(\d)+\,(\d)+', inputStr))
...
def MainCmdline(CmdWidgetDict):
    ...
    vulcmdlists= ['echo vulcan -v --eris --db --user jiag --dry-run','./msgloop.py']
    
    # Shared data structure: Event/Queue 
    findcr_evt_completed = Event()

    # 1. check if needs to find CL first  
    if CmdWidgetDict.get('FindRev').range.get():
        if isRevRange(CmdWidgetDict.get('FindRev').range.get()):
            print('****** In FindRev ****** ')
            ...
            # invalid param error 
            raise ValueError("INVALID '--revision-range'! Expect: 'digits,digits' format\n")
    else:
...
WidgetAll.update({'FindRev':DicCmdFindRevMenu(root,picks=RevDict,rowNum=startRow)})
