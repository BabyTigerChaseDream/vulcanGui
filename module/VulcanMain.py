...
startRow += 1
CLList=[]
Button(root,text='CLRun', command=lambda :newThreadCmd(MyText=TextEditor(), cmdline=shCmdline, handler='CL',OutData=CLList)).grid(row=startRow)

for item in CLList:
    print('main:',item)

startRow += 1
Button(root,text='WebRun', command=lambda :newThreadCmd(MyText=TextEditor(), cmdline=shCmdline, handler='Web')).grid(row=startRow)
